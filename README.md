...
匿名 JWT 発行は認証不要なので、無制限に許すと自動化スクリプトで匿名 `user_id` を量産され、偽いいねや偽ビューに使われる脆弱性があった。投稿系も無制限だとスパム投稿で皿（コンベア）を埋め尽くされる懸念があった。
...
- **2つの limiter を用途別に分離**（[`src/middleware/rateLimit.ts`](src/middleware/rateLimit.ts)）：
  ...
  - `writeLimiter` — 全書き込み系 (POST 投稿・コメント・返信・箱・いいね・ビュー) に適用、**ユーザーIDベース 60 req / 1 分**。NAT 経由で同一 IP を共有する複数ユーザーが誤って巻き込まれない設計
